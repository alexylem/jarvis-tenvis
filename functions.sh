...
tenvis_settings () {
    ...
    local data=$2}
    wget -qO /dev/null "http://$tenvis_ip:$tenvis_port/goform/formSet${type}Cfg" --post-data "$data" --user=$tenvis_user --password=$tenvis_password --auth-no-challenge
}
tenvis_settings_led () {
...
tenvis_action () { # action $code $value
    wget -qO /dev/null "http://$tenvis_ip:$tenvis_port/media/?action=cmd&code=$1&value=$2" --user=$tenvis_user --password=$tenvis_password --auth-no-challenge
}
